...
const Vector NPC_MAXS					= Vector( 16, 16, 88 ); //60 in quake 2
	...
	void GunnerGrenade( int iFrame )
	{
		...
		if( m_bRerelease )
		{
			...
			if( M_CalculatePitchToFire(vecTarget, vecMuzzle, vecAim, 600, 2.5, false) )
				monster_fire_weapon( q2::WEAPON_GRENADE, vecMuzzle, vecAim, GRENADE_DMG, GRENADE_SPEED, crandom_open() * 10.0, Math.RandomFloat(0.0, 1.0) * 10.0 );
			else
			{
				//normal shot
				Math.MakeVectors( pev.angles );
				vecAim = g_Engine.v_forward;
				monster_fire_weapon( q2::WEAPON_GRENADE, vecMuzzle, vecAim, GRENADE_DMG, GRENADE_SPEED, crandom_open() * 10.0, 200.0 + (crandom_open() * 10.0) );
			}
		}
		else
		{
